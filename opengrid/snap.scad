...
nub_directional = false; // removes strain relief from the 'back' nub
...
module nub(pos) {
    ...
    if (pos == "front") { move([0, -tileY/2, 0]) xrot(90) yrot(  0) draw(); }
    if (pos == "back")  { move([0,  tileY/2, 0]) xrot(90) yrot(180) draw(); }
    // TODO: larger bump when directional
}

// 'lite' only
module relief(pos) {
    r = 0.6 / 2; // corresponds with draw_v.d
    ...
    if (pos == "front") { zrot(180) move([0, tileY/2, 0]) draw(); }
    if (pos == "back")  { zrot(  0) move([0, tileY/2, 0]) draw(); }
}
...
module snap() {
    ...
    difference() {
        union() {
            ...
            if (nub_front) nub("front");
            if (nub_back) nub("back");
        }
        // Cut out the strain reliefs
        if (nub_right) relief("right");
        if (nub_left) relief("left");
        if (nub_front) relief("front");
        if (nub_back) relief("back");
    }
}
