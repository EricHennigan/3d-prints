...
// Control placement of plugs: none (no plugs), top (fill top notches only), bottom (fill bottom
// notches only), both (continuous plug)
end_cap_plug_placement = "bottom"; // [none, top, bottom, both]
  ...
  difference() {
    ...
    if (!alternate_pattern || num_front_columns == 1) {
      ...
      for (i = [0:num_front_columns - 1]) {
        x = (upright_width - ( (num_front_columns - 1) * column_spacing + peg_hole_width)) / 2 + i * column_spacing;
        start_y = (i % 2 == 0) ? 20 : 40;
    ...
    if (!alternate_pattern || num_side_columns == 1) {
      ...
      for (i = [0:num_side_columns - 1]) {
        z = (upright_depth - ( (num_side_columns - 1) * column_spacing + peg_hole_width)) / 2 + peg_hole_width / 2 + i * column_spacing;
        start_y = (i % 2 == 0) ? 20 : 40;
        for (y = [start_y:40:upright_height - peg_hole_height / 2]) {
          if (y + peg_hole_height / 2 <= upright_height - top_margin)
...
module front_beam_with_pegs(y_position, include_odd_columns, include_even_columns) {
    ...
    if (add_beam_ventilation) {
      ...
      for (col = [0:num_cols - 1])
        for (row = [0:num_rows - 1]) {
          ...
          if (
            hx - beam_pattern_size / 2 >= margin && hx + beam_pattern_size / 2 <= beam_width - margin && hz + beam_pattern_size / 2 <= -margin && hz - beam_pattern_size / 2 >= -beam_length + margin
          )
  ...
  for (i = [0:num_front_columns - 1]) {
    col_idx = i + 1;
    if (
      (col_idx % 2 == 1 && include_odd_columns) || (col_idx % 2 == 0 && include_even_columns)
    ) {
      x = (upright_width - ( (num_front_columns - 1) * column_spacing + peg_hole_width)) / 2 + i * column_spacing;
  ...
  if (alternate_pattern && num_front_columns > 1) {
    ...
    for (i = [0:num_front_columns - 1]) {
      ...
      if (
        (col_idx % 2 == 1 && back_include_odd) || (col_idx % 2 == 0 && back_include_even)
      ) {
...
module end_cap() {
  ...
  union() {
    ...
    if (end_cap_plug_placement == "top" || end_cap_plug_placement == "both") {
      difference() {
        union() {
          y = upright_height + end_cap_insert_height + end_cap_height - peg_hole_height / 2;
          for (i = [0:num_front_columns - 1]) {
            x = (upright_width - ( (num_front_columns - 1) * column_spacing - peg_hole_width)) / 2 + i * column_spacing - shell_thickness;
            translate([x, y, end_cap_tolerance])
              linear_extrude(height=shell_thickness)
                peg_plug();
            translate([x, y, upright_depth - shell_thickness - end_cap_tolerance])
              linear_extrude(height=shell_thickness)
                peg_plug();
          }
          for (i = [0:num_side_columns - 1]) {
            z = (upright_depth - ( (num_side_columns - 1) * column_spacing + peg_hole_width)) / 2 + i * column_spacing + shell_thickness;
            translate([end_cap_tolerance, y, z])
              rotate([0, 90, 0]) linear_extrude(height=shell_thickness)
                  peg_plug();
            translate([upright_width - shell_thickness - end_cap_tolerance, y, z])
              rotate([0, 90, 0]) linear_extrude(height=shell_thickness)
                  peg_plug();
          }
        }
        cutoff = peg_hole_height;
        translate([0, upright_height + end_cap_insert_height - cutoff, 0])
          cube([upright_width, end_cap_height + cutoff, upright_depth]);
      }
    }
    if (end_cap_plug_placement == "bottom" || end_cap_plug_placement == "both") {
      difference() {
        union() {
          y = upright_height + end_cap_insert_height - peg_hole_height / 2;
          for (i = [0:num_front_columns - 1]) {
            x = (upright_width - ( (num_front_columns - 1) * column_spacing - peg_hole_width)) / 2 + i * column_spacing - shell_thickness;
            translate([x, y, end_cap_tolerance])
              linear_extrude(height=shell_thickness)
